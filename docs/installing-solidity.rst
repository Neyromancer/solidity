...
- The content is mirrored to https://solc-bin.ethereum.org where it can be easily downloaded over
...
- The binary is also mirrored at https://solc-bin.ethereum.org/emscripten-wasm32/solc-emscripten-wasm32-v0.7.4+commit.3f05b770.js.
...
.. warning::
    ...
    If you are using it, please switch to https://solc-bin.ethereum.org, which is a drop-in replacement.
    This allows us to make changes to the underlying hosting in a transparent way and minimize
    disruption. Unlike the ``ethereum.github.io`` domain, which we do not have any control over,
    ``solc-bin.ethereum.org`` is guaranteed to work and maintain the same URL structure in the long-term.
